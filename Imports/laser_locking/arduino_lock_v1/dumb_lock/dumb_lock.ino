...
void rampCycle(bool find_lock_point) {
  ...
  if(find_lock_point){
    lock_point= ToBits(2.0) ;//ToVoltage(max_sig)/2);
    lock_point_offset = max_sig_out+(params.fwhm/2);
    params.output_offset = ZEROV ; //lock_point_offset;
  }
}
...
void loop() {
  ...
  float err_curr = ((float)(in0 - lock_point)); //current signal reading, in ??
