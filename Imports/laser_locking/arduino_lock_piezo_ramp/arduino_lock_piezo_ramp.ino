...
#define OUTPUT_CHAN 0
#define SWITCH_CHAN 1
...
#define OUTPUT_MAX 39322 //1V
#define OUTPUT_MIN 26214 //-1V
...
void processParams() {
  ...
  if(params.scan_state==STATE_SCANNING){
    analog.write(1,ZEROV); 
  }
}
int rampCycle(bool find_lock_point) {
  long ramp_time;
  long start_time;
  ...
  while(ramp_time<half_period){
    ...
    if(find_lock_point){
      sig_in = analog.read(1,false);
      if(sig_in<params.lock_point){
        lock_start_voltage = out0;
  ...
  if(params.scan_state == STATE_LOCKING) {
    ...
    */
    ...
    for(int i=0;i<params.n_averages;i++){
      in0+=analog.read(1,false)/params.n_averages;
    }
    ...
    if(abs(accumulator2) > ACCUMULATOR2_MAX) { //if accumulator 2 gets too big, out of lock
      ...
      relock_trip = relock_trip + 1;
      //Serial.print(String(relock_trip));
   ...
   out1 -= params.gain_p*error_signal+accumulator2+accumulator; // note the overall sign on the gain
   analog.write(1,limitOutput(out1));  //WRITE OUT THE CORRECTION SIGNAL
