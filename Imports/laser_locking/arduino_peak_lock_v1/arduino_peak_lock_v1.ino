...
void processParams() {
  ...
  fwhm_bits = ToBits(params.fwhm)-ZEROV;
  fwhm_bits_o2 = fwhm_bits/2;
  //accumulator = 0.0;
  //accumulator2=0.0;
  if(params.scan_state==STATE_LOCKING){
    accumulator = 0.0;
    accumulator2=0.0;
    long avg_lock_point_start = 0;
    for(int i=0;i<8;i++){
      rampCycle(true);
      avg_lock_point_start += lock_start_voltage/8;
    }
    lock_start_voltage = avg_lock_point_start;
  }
}
void rampCycle(bool find_lock_point) {
  ...
  if(find_lock_point){
    lock_start_voltage = max_sig_out-fwhm_bits_o2;
...
void loop() {
  ...
  if(params.scan_state == STATE_LOCKING) {
    ...
    */
    ...
    analog.write(0,out0+fwhm_bits);
    long right_sig = analog.read(0,false);

    float err_curr = left_sig - right_sig;
   ...
   analog.write(0,limitOutput(out0-fwhm_bits_o2));  //WRITE OUT THE CORRECTION SIGNAL
